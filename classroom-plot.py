...
# -------------------- 中文字体设置 --------------------
plt.rcParams['font.sans-serif'] = ['SimHei', 'DejaVu Sans', 'Arial Unicode MS', 'WenQuanYi Micro Hei', 'Noto Sans CJK']
...
def main(json_path, output_path):
    ...
    if total_answers > 0:
        overall_avg = sum(score_map[i] * total_counts[i] for i in range(5)) / total_answers
    else:
        overall_avg = 0.0
    ...
    ax2.plot(centers, avg_scores,
             color='red', marker='o', linewidth=2, markersize=6,
             label='班级平均成绩')

    # ---------- 为每个班级标注具体平均分 ----------
    for xi, avg in zip(centers, avg_scores):
        ax2.text(xi, avg + 2, f'{avg:.1f}', ha='center', va='bottom',
                 fontsize=8, color='red', fontweight='bold')

    # 添加全员平均成绩水平线
    ax2.axhline(y=overall_avg, color='red', linestyle='--', linewidth=1.5,
    ...
    ax2.set_ylim(0, 100)
    # 右侧图例放在图外下方
    ax2.legend(loc='upper left', bbox_to_anchor=(0.7, -0.15), borderaxespad=0)
